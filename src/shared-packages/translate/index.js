...
export const translate = async (from, to, text, context) => {
    try {
        ...
        const res = await get(`https://translate.googleapis.com/translate_a/single?client=gtx&sl=${from}&tl=${to}&dt=t&dt=bd&dt=rm&dj=1&source=input&q=${encodeURIComponent(text)}`);
